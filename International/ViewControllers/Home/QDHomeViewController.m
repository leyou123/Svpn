...
- (void)setPremiumView {
    self.premiumView = [[QDGetPremiumView alloc] initWithFrame:CGRectZero clickAction:^(NSInteger status) {
        if (status == 1) {
            ...
            if ([QDVersionManager.shared.versionConfig[@"show_payment_ad"] intValue] == 1) {
                self.canShowAds = YES;
            }else {
                self.canShowAds = NO;
            }
...
- (void) setupLine {
    ...
    self.lineButton = [[QDLineButtonView alloc] initWithFrame:CGRectZero clickBlock:^{
        if (QDConfigManager.shared.activeModel.member_type == 1) {
            if ([QDVersionManager.shared.versionConfig[@"show_node_ad"] intValue] == 1) {
                self.canShowAds = YES;
            }else {
                self.canShowAds = NO;
            }
        ...
        [QDDialogManager showDialog:NSLocalizedString(@"Connect_fail", nil) message:NSLocalizedString(@"Connect_fail_info", nil) ok:NSLocalizedString(@"Dialog_Ok", nil) cancel:NSLocalizedString(@"Dialog_Cancel", nil) okBlock:^{
            ...
            [QDVPNManager.shared reStartInstallConfig:^(NSError * _Nonnull error) {
                if (error) {
                    ...
                    if ([QDVersionManager.shared.versionConfig[@"show_link_ad"] intValue] == 1) {
                        // 显示插屏广告
                        [self showInsertAd];
                    }
...
//
//    }
    if ([QDVersionManager.shared.versionConfig[@"show_connect_success_ad"] intValue] == 1) {
        self.canShowAds = YES;
    }else {
        self.canShowAds = NO;
    }
...
- (void) onScanButtonClicked {
    ...
    if (QDConfigManager.shared.activeModel.member_type == 1) {
        ...
        if ([QDVersionManager.shared.versionConfig[@"show_qrcode_ad"] intValue] == 1) {
            self.canShowAds = YES;
        }else {
            self.canShowAds = NO;
        }
...
- (void) onBindAction {
    if ([QDVersionManager.shared.versionConfig[@"show_bing_email_ad"] intValue] == 1) {
        self.canShowAds = YES;
    }else {
        self.canShowAds = NO;
    }
