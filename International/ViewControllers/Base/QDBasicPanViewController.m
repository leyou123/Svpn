...
#import "QDBasicPanViewController.h"

#define time 0.2f

@interface QDBasicPanViewController () {
...
@implementation QDBasicPanViewController
...
-(UIImage*)imageByCropping:(UIView*)imageToCrop toRect:(CGRect)rect
{
    ...
    CGSize pageSize = CGSizeMake(scale*width, scale*height);
...
#pragma mark 手势
- (void) handleSwipe:(UISwipeGestureRecognizer*)recongizer {
    NSUInteger index = [self.tabBarController selectedIndex];
    switch (recongizer.direction) {
        case UISwipeGestureRecognizerDirectionLeft:
        {
            if (index == self.tabBarController.viewControllers.count-1) return;
...
- (void) selectTabBar:(NSInteger) index {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:kNotificationtabChanged object:nil];
}
